...
#'    ensures proper indentation of XML file produced. Additional arguments can be
#'    specified and will be passed to the XML encoding.
#'  }
...
ISOAbstractObject <- R6Class("ISOAbstractObject",
  ...
  private = list(
    ...
    xmlHeader = function(isoCompliant = NA,
      ...
      txt <- paste(txt, ISOCompliance, sep="\n")
      if(!is.na(inspireCompliant)){
        txt <- paste(txt, INSPIRECompliance, sep="\n")
        txt <- paste(txt, INSPIRECompleteness, sep="\n")
        txt <- paste(txt, INSPIREReport, sep="\n")
      }
      txt <- paste(txt, "-->", sep="\n")
      txt <- paste(txt, "<!-- ", sep="\n")
  ...
  public = list(
    ...
    encode = function(addNS = TRUE, validate = TRUE, strict = FALSE, inspire = FALSE,
      ...
      #add header
      if(self$isDocument()){
        xmlHeader <- private$xmlHeader(compliant)
        if(inspire){
          xmlHeader <- private$xmlHeader(compliant$ISO,
                                         compliant$INSPIRE$validity$completeness$status, 
                                         compliant$INSPIRE$validity$completeness$percentage,
                                         compliant$INSPIRE$url)
        }
        outbuf <- xmlOutputBuffer("", header = xmlHeader)
